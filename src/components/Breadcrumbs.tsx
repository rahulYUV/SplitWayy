...
export function Breadcrumbs({ userName, currentPage, className = "" }: BreadcrumbsProps) {
    return (
        <div className={`flex flex-col ${className}`}>
            <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter flex items-center gap-3 drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)]">
                ...
                {currentPage ? (
                    <>
                        <span className="opacity-40 uppercase">{userName}</span>
                        <span className="text-[#32dd9e] text-2xl font-normal opacity-100">{">"}</span>
